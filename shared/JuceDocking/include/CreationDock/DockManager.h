...
public:
    ...
    void closePanel(const juce::String& id);
    void showPanel(const juce::String& id, DockTargetZone fallbackZone = DockTargetZone::CenterTab);
    void activatePanel(const juce::String& id);
    bool isPanelOpen(const juce::String& id) const;
...
private:
    ...
    std::vector<std::unique_ptr<DockPanel>> hiddenPanels;
    std::vector<std::pair<juce::String, DockTargetZone>> defaultLayout;
    std::vector<std::pair<juce::String, DockTargetZone>> lastKnownLayout;
    ...
    DockTargetZone getRememberedZone(const juce::String& id, DockTargetZone fallback) const;
    void rememberZone(const juce::String& id, DockTargetZone zone);
